...
- **`docs/`** — Astro + Starlight documentation site (https://tg-bot-sdk.website). Interactive playground (Alpine.js + Monaco Editor). `/llms.txt` and `/llms-full.txt` are generated from the guide pages by `src/pages/llms*.txt.ts`.
- **`example/`** — Live demo: a Cloudflare Worker (`src/worker.ts`) hosting several demo bots (`src/bots/`) behind one bot token, switched via an inline menu with per-chat state in KV. Deployed by `.github/workflows/deploy-demo.yml`. `src/polling/` runs the same bots locally with long polling. `Makefile` wraps local dev (`wrangler dev`, tunnel, set-webhook).
